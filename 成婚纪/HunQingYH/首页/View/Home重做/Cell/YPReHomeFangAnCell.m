...
- (void)setColView:(UICollectionView *)colView{
    ...
    layout.itemSize = CGSizeMake(250, 210);
    layout.minimumInteritemSpacing = 10;
    layout.minimumLineSpacing = 10;
    layout.scrollDirection = UICollectionViewScrollDirectionHorizontal;

    if (!colView) {
        colView = [[UICollectionView alloc]initWithFrame:CGRectMake(10, 10, ScreenWidth-20, 210) collectionViewLayout:layout];
    }
...
//    return self.planList.count;
    return self.demoPlanList.count;
}

- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    
//    YPGetWebPlanList *plan = self.planList[indexPath.item];
    YPGetDemoPlanList *plan = self.demoPlanList[indexPath.item];
...
//    [cell.iconImgV sd_setImageWithURL:[NSURL URLWithString:plan.ImgUrl] placeholderImage:[UIImage imageNamed:@"占位图"]];
    [cell.iconImgV sd_setImageWithURL:[NSURL URLWithString:plan.ShowImg] placeholderImage:[UIImage imageNamed:@"占位图"]];
//    cell.titleLabel.text = plan.PlanName;
    if (plan.PlanTitle.length > 0) {
        cell.titleLabel.text = plan.PlanTitle;
    }else{
        cell.titleLabel.text = @"无标题";
    }
    //5-9 修改 显示关键字
    if (plan.PlanKeyWord.length > 0) {
        cell.collectionTitle.text = plan.PlanKeyWord;
    }else{
        cell.collectionTitle.text = @"无关键字";
    }
    //6-11 价格
    cell.priceLabel.text = [NSString stringWithFormat:@"¥%d",28000];
