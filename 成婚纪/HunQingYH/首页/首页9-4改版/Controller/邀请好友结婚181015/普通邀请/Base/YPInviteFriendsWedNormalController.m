...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        [_submitBtn setBackgroundImage:[self gradientImageWithBounds:CGRectMake(0, 0, ScreenWidth-36, 48) andColors:@[RGBA(255, 163, 89, 1),RGBA(255, 93, 118, 1)] andGradientType:1] forState:UIControlStateNormal];
        ...
        cell.colCellClick = ^(NSString *sectionName, NSIndexPath *indexPath) {
            if (!UserId_New) {
                ...
                if ([sectionName isEqualToString:@"开通VIP邀请"]) {
                    ...
                    [self showShareSDKWithURL:[NSString stringWithFormat:@"http://www.chenghunji.com/Capital/BeiHun?UserId=%@",UserId_New] Title:@"送您一份新婚礼，快去婚礼桥APP领取！" Text:@"婚庆、婚纱，全部花多少返多少！婚礼对戒等更多豪礼速来领！"];
...
//        }];
        [BRDatePickerView showDatePickerWithTitle:@"请选择新人婚期" dateType:BRDatePickerModeDate defaultSelValue:@"" minDate:[NSDate date] maxDate:nil isAutoSelect:NO themeColor:nil resultBlock:^(NSString *selectValue) {
            self.dateStr = selectValue;
            [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
        }];
