...
@interface YPPassengerDistributionController ()<UITableViewDelegate,UITableViewDataSource,CJAreaPickerDelegate>
...
- (void)setupUI{
    ...
    if (!self.tableView) {
        if ([self.typeStr isEqualToString:@"Me"]) {
...
//    UIButton *submitBtn = [[UIButton alloc]init];
//    [submitBtn setTitle:@"提交申请" forState:UIControlStateNormal];
//    [submitBtn setTitleColor:CHJ_RedColor forState:UIControlStateNormal];
//    [submitBtn addTarget:self action:@selector(submitBtnClick) forControlEvents:UIControlEventTouchUpInside];
//    [_navView addSubview:submitBtn];
//    [submitBtn mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    if (tableView == self.tableView) {
        if (indexPath.section == 0) {
            ...
            if (self.listMarr.count > 0) {
                ...
                YPPassageDistributeListCell *cell = [YPPassageDistributeListCell cellWithTableView:tableView];
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
                ...
                cell.profession.text = listModel.Identity;
                if (listModel.WeddingTime.length > 0) {
                    cell.wedDate.text = listModel.WeddingTime;
                }else{
                    cell.wedDate.text = @"无婚期";
                }
                if (listModel.Time.length > 0) {
                    cell.shangchuan.text = listModel.Time;
                }else{
                    cell.shangchuan.text = @"无上传时间";
                }
                if (listModel.Area.length > 0) {
                    cell.quyu.text = listModel.Area;
                }else{
                    cell.quyu.text = @"无区域";
                }
                cell.zhuoshu.text = [NSString stringWithFormat:@"%zd",listModel.TablesNumber.integerValue];
                cell.canbiao.text = [NSString stringWithFormat:@"%@",listModel.MealMark];
                
                cell.lookBtn.tag = indexPath.section + 1000;
                [cell.lookBtn addTarget:self action:@selector(lookBtnClick:) forControlEvents:UIControlEventTouchUpInside];
                
                return cell;
                
//                if (indexPath.row == 0) {
//                    YPPassegerDistribute1Cell *cell = [YPPassegerDistribute1Cell cellWithTableView:tableView];
//                    cell.selectionStyle = UITableViewCellSelectionStyleNone;
//                    if (listModel.Name.length > 0) {
...
//                    }
//                    cell.tableCount.text = [NSString stringWithFormat:@"%zd",listModel.TablesNumber.integerValue];
//                    cell.canbiao.text = [NSString stringWithFormat:@"%@元",listModel.MealMark];
//                    if (listModel.DistributionType.integerValue == 0) {//0未分配，已分配
//                        cell.tagImgV.hidden = NO;
//                        if ([listModel.Identity isEqualToString:@"酒店"]) {
//                            [cell.tagImgV setImage:[UIImage imageNamed:@"keyuan_Jiudian"]];
//                        }else if ([listModel.Identity isEqualToString:@"婚纱"]){
//                            [cell.tagImgV setImage:[UIImage imageNamed:@"keyuan_Hunsha"]];
//                        }else if ([listModel.Identity isEqualToString:@"婚庆"]){
//                            [cell.tagImgV setImage:[UIImage imageNamed:@"keyuan_Hunqing"]];
//                        }
//                    }else{
...
//                    YPPassagerDistribute2Cell *cell = [YPPassagerDistribute2Cell cellWithTableView:tableView];
//                    cell.selectionStyle = UITableViewCellSelectionStyleNone;
//                    if (listModel.WeddingTime.length > 0) {
//                        cell.wedDate.text = listModel.WeddingTime;
//                    }else{
...
//                    }
//                    if (listModel.Time.length > 0) {
//                        cell.upTime.text = listModel.Time;
//                    }else{
//                        cell.upTime.text = @"无上传时间";
//                    }
//                    return cell;
//                }
            }else{
