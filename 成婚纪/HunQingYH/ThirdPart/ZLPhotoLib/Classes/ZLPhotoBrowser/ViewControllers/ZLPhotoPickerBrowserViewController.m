...
- (void)showToView{
    ...
    if (self.status == UIViewAnimationAnimationStatusFade){
        ...
        if (self.userScrollView && self.userScrollView.contentOffset.y >= 0) {
            tempF.origin.y -= self.userScrollView.contentOffset.y + NAVIGATION_BAR_HEIGHT;
        }
...
-(void)setPageLabelPage:(NSInteger)page{
    self.pageLabel.text = [NSString stringWithFormat:@"%ld / %lu",page + 1, (unsigned long)self.photos.count];
    if (self.isPush) {
