...
+ (NSString *)postRequestWithURL: (NSString *)url  // IN
                     ...
                     picFilePath: (UIImage *)picFilePath  // IN
                     picFileName: (NSString *)picFileName;  // IN
{
    
    [EasyShowLodingView showLodingText:@"图片正在上传中,请稍候..."];
    ...
    if(picFilePath){
        ...
        UIImage *image=picFilePath;
        //判断图片是不是png格式的文件
        if (UIImagePNGRepresentation(image)) {
            //返回为png图像。
            
            data =[UIImage resetSizeOfImageData:image];// UIImagePNGRepresentation(image);
        }else {
            //返回为JPEG图像。
            data = [UIImage resetSizeOfImageData:image];//UIImageJPEGRepresentation(image, 1.0);
        }
    }
    //http body的字符串
...
//         postNotificationName:@"upLoadImageSuccess" object:self];
        
        [EasyShowTextView showSuccessText:@"图片上传成功!"];
    }else{
        
         [EasyShowTextView showErrorText:@"图片上传失败，请稍后重试"];
        
//        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"温馨提示" message:@"图片上传失败，请稍后重试" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
//        
//        [alertView show];
