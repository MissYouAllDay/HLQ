...
- (void)drawRect:(CGRect)rect1 {
    //计算每位验证码/密码的所在区域的宽和高
    CGRect rect = CGRectMake(0,0,TFW,TFH);
...
- (void)drawBottomLineWithRect:(CGRect)rect1 andIndex:(int)k{
    ...
    if ( k<=self.text.length ) {
        CGContextSetStrokeColorWithColor(context,MainColor.CGColor);//底部颜色
        CGContextSetFillColorWithColor(context,MainColor.CGColor);//内容的颜色
    }else{
        CGContextSetStrokeColorWithColor(context,[UIColor lightGrayColor].CGColor);//底部颜色
        CGContextSetFillColorWithColor(context,MainColor.CGColor);//内容的颜色
    }
