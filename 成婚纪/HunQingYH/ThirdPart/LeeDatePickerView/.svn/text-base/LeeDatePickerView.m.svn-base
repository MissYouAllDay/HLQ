...
@property (nonatomic, strong) UIView * titleContentView; // 标题区
...
static CGFloat sContentHeight = 360.0f; // 内容显示区 高度
static CGFloat sTitleContentHeight = 60.0f; // 内容显示区 高度
static CGFloat sContentHeadHeight = 60.0f; // 内容显示区 头部高度
...
-(instancetype)init{
    self = [super init];
    if (self) {
        self.frame = CGRectMake(0, NAVIGATION_BAR_HEIGHT, ScreenWidth, ScreenHeight-NAVIGATION_BAR_HEIGHT);
...
-(void)setUpView{
#pragma mark 内容显示区
    self.contentView = [[UIView alloc]initWithFrame:CGRectMake(0, -sContentHeight, self.bounds.size.width, sContentHeight)];
    self.contentView.backgroundColor = [UIColor whiteColor];
    [self addSubview:self.contentView];

#pragma mark 标题区 18-10-30
    self.titleContentView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.bounds.size.width, sTitleContentHeight)];
    self.titleContentView.backgroundColor = [UIColor whiteColor];
    ...
    [label mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.mas_equalTo(self.titleContentView);
    }];
    ...
    [self.titleContentView addSubview:closeBtn];
    ...
    [resetBtn setTitleColor:RGBS(153) forState:UIControlStateNormal];
    resetBtn.titleLabel.font = kFont(12);
    resetBtn.layer.cornerRadius = 13;
    resetBtn.clipsToBounds = YES;
    resetBtn.layer.borderColor = RGBS(221).CGColor;
    resetBtn.layer.borderWidth = 1;
    [resetBtn addTarget:self action:@selector(resetBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.titleContentView addSubview:resetBtn];
...
#pragma mark 内容显示区头部
    self.contentHeadView = [[LeeDatePickerHeadView alloc]initWithFrame:CGRectMake(0, sTitleContentHeight, self.bounds.size.width, sContentHeadHeight)];
...
#pragma mark 时间选择区
    self.datePickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(0, sContentHeadHeight+sTitleContentHeight, [UIScreen mainScreen].bounds.size.width, sContentCenterHeight)];
...
#pragma mark 内容页底部
    self.contentBottomView = [[UIView alloc]initWithFrame:CGRectMake(0, sContentHeadHeight + sContentCenterHeight + sTitleContentHeight, self.bounds.size.width, sContentBottomHeight)];
...
-(void)selectPickerDate:(NSDate *)date{
    ...
    self.yearIndex = [self.yearArray indexOfObject:[NSString stringWithFormat:@"%ld", (long)comp.year]];
...
-(void)showContentView:(BOOL)show{
    if (show) {
        [UIView animateWithDuration:0.3 animations:^{
            self.contentView.frame = CGRectMake(0, 0, ScreenWidth, sContentHeight);
        }];
    }else{
        [UIView animateWithDuration:0.3 animations:^{
            self.contentView.frame = CGRectMake(0, - sContentHeight, self.bounds.size.width, sContentHeight);
