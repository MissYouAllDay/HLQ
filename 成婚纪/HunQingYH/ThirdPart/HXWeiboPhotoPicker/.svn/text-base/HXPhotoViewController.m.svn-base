...
- (void)pushAlbumList:(UIButton *)button {
    ...
    if (button.selected) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            self.albumView.frame = CGRectMake(0, NAVIGATION_BAR_HEIGHT, self.view.frame.size.width, 340);
...
- (UINavigationBar *)navBar {
    if (!_navBar) {
        _navBar = [[UINavigationBar alloc] initWithFrame:CGRectMake(0, 20, self.view.hx_w, NAVIGATION_BAR_HEIGHT)];
...
- (void)setup {
    ...
    if (!self.manager.singleSelected) {
        self.collectionView.contentInset = UIEdgeInsetsMake(spacing + NAVIGATION_BAR_HEIGHT, 0, spacing + 50, 0);
        ...
        if (self.manager.type == HXPhotoManagerSelectedTypeVideo) {
            ...
            self.collectionView.contentInset = UIEdgeInsetsMake(spacing + NAVIGATION_BAR_HEIGHT, 0,  spacing, 0);
            self.collectionView.scrollIndicatorInsets = self.collectionView.contentInset;
        }
...
- (UIView *)albumsBgView {
    if (!_albumsBgView) {
        _albumsBgView = [[UIView alloc] initWithFrame:CGRectMake(0, NAVIGATION_BAR_HEIGHT, self.view.frame.size.width, self.view.frame.size.height - NAVIGATION_BAR_HEIGHT)];
