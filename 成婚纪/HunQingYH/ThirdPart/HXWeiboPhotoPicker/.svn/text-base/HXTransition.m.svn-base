...
//        }
    ...
    tempView.center = CGPointMake(containerView.frame.size.width / 2, (height - NAVIGATION_BAR_HEIGHT) / 2 + NAVIGATION_BAR_HEIGHT);
    CGRect rect = [toVC.collectionView convertRect:cell.frame toView:[UIApplication sharedApplication].keyWindow];
    if (cell) {
        if (rect.origin.y < NAVIGATION_BAR_HEIGHT) {
