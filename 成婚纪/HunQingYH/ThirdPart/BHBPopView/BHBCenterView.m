...
- (void)layoutBtnsWith:(NSArray *)items isMore:(BOOL)isMore{
    ...
    for (int i = 0; i < items.count; i ++) {
        ...
        [btn.imageView setContentMode:UIViewContentModeScaleAspectFit];//UIViewContentModeCenter
...
//        CGFloat x = (i % 2) * self.frame.size.width / 2.0;
//        CGFloat y = (i / 2) * self.frame.size.height / 2.0;

//        CGFloat x = (i % 2) * ((self.frame.size.width - 35*2)/3.0 + 35);
        CGFloat x = 0;
        if ([btn.titleLabel.text isEqualToString:@"视频"]) {
            x = self.frame.size.width*0.5 - 150;
        }else if ([btn.titleLabel.text isEqualToString:@"照片"]){
            x = self.frame.size.width*0.5 + 20;
        }
//        CGFloat y = self.frame.size.height - 40 - 35;
        CGFloat y = self.frame.size.height - 150;//6-26 修改
