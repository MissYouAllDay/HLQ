...
//    }else{
        ...
        YPGetWebSupplierList *gysmodel = self.gysMarr[indexPath.item];
        ...
        [cell.iconImgV sd_setImageWithURL:[NSURL URLWithString:gysmodel.ImgUrl] placeholderImage:[UIImage imageNamed:@"占位图"]];
...
//        if (gysmodel.CaseCount.length > 0){
            cell.anliCount.text = gysmodel.CaseCount;
//        }
//        if (gysmodel.StateCount.length > 0){
            cell.zhuangtaiCount.text = gysmodel.StateCount;
//        }
    
    if ([gysmodel.IsSign integerValue] == 1){//0未签约 1已签约
        cell.danbaoImgV.hidden = NO;
    }else{
        cell.danbaoImgV.hidden = YES;
    }

    if ([gysmodel.IsSign integerValue] == 1){//0未送礼 1已送礼
        cell.giftImgV.hidden = NO;
    }else{
        cell.giftImgV.hidden = YES;
    }
        return cell;
//    }
