...
- (void)setUpPadding{
    ...
    for (NSString * titleString in self.titileArray) {
        ...
        CGSize currentButtonSize = [titleString boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, kViewHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:self.highLightFontSize weight:UIFontWeightSemibold]} context:nil].size;
...
- (void)setUpTitleLabels{
    ...
    for (int index = 0; index < self.titileArray.count; index++) {
        ...
        if (index == 0) {
            ...
            self.selectedLabel.textColor = RGBS(51);
            self.selectedLabel.font = [UIFont systemFontOfSize:self.highLightFontSize weight:UIFontWeightSemibold];
        }
    }
    ...
    self.boderView.layer.cornerRadius = 2;
    self.boderView.clipsToBounds = YES;
}
