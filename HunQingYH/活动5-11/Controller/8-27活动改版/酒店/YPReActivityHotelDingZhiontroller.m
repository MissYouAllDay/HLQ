//
//  YPReActivityHotelDingZhiontroller.m
//  HunQingYH
//
...
@interface YPReActivityHotelDingZhiontroller ()<UITableViewDelegate,UITableViewDataSource,YPAddRemarkDelegate,CJAreaPickerDelegate>
...
@implementation YPReActivityHotelDingZhiontroller{
    UIView *_navView;
    //数据库
    FMDatabase *dataBase;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0) {
        ...
        if (self.zhuoshu.length > 0) {
            cell.contentLabel.text = self.zhuoshu;
            cell.contentLabel.textColor = BlackColor;
        }else{
            cell.contentLabel.text = @"请填写婚宴桌数";
            cell.contentLabel.textColor = LightGrayColor;
        }
        ...
        if (self.jiage.length > 0) {
            ...
            cell.contentLabel.text = @"请填写平均每桌用餐价格";
            cell.contentLabel.textColor = LightGrayColor;
        }
...
- (void)yp_PersonOrder:(NSString *)content AndTag:(NSString *)tag{
    if ([tag isEqualToString:@"zhuoshu"]) {
        self.zhuoshu = content;
    }else if ([tag isEqualToString:@"jiage"]) {
        self.jiage = content;
...
- (void)AddFacilitatorCustomMade{
    ...
    if (self.zhuoshu.length > 0) {
        params[@"TableNumber"] = self.zhuoshu;
    }else{
        params[@"TableNumber"] = @"";
    }
    if (self.beizhu.length > 0) {
