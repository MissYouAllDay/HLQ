...
@interface YPArrangeController ()<ZJScrollPageViewDelegate,UITableViewDataSource,UITableViewDelegate>
...
@implementation YPArrangeController{
    UIView *_navView;
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshData) name:@"LcTabBarDidClickNotification" object:nil];
...
- (void)setupUI{
    ...
    if (JiuDian(Profession_New)){//酒店只有档期
        self.dangqiVC.view.frame =CGRectMake(0, NAVIGATION_BAR_HEIGHT, ScreenWidth, ScreenHeight-NAVIGATION_BAR_HEIGHT-49);
        ...
        ZJContentView *content = [[ZJContentView alloc] initWithFrame:CGRectMake(0.0, NAVIGATION_BAR_HEIGHT, ScreenWidth, ScreenHeight - NAVIGATION_BAR_HEIGHT - TabBarHeight) segmentView:segment parentViewController:self delegate:self];
...
//    }];
    ...
    [_hunqingView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(ScreenHeight-NAVIGATION_BAR_HEIGHT-1-TabBarHeight);
    }];
...
- (void)setHunQingList{
    ...
    if (self.listMarr.count > 0) {
        ...
        if (!self.tableView) {
            self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, NAVIGATION_BAR_HEIGHT+1, ScreenWidth, ScreenHeight-NAVIGATION_BAR_HEIGHT-1-TabBarHeight) style:UITableViewStylePlain];
        }
