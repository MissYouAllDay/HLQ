...
#import "YPReMeGuanzhuController.h"
...
@interface YPReMeGuanzhuController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate>
...
@implementation YPReMeGuanzhuController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.listModel = list;
    cell.guanzhuBtn.tag = indexPath.row + 1000;
    [cell.guanzhuBtn addTarget:self action:@selector(guanzhuBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    return cell;
}
...
#pragma mark - target
- (void)guanzhuBtnClick:(UIButton *)sender{
    NSLog(@"取消关注");
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        YPUserFollowInfoList *list = self.listMarr[alertView.tag - 1000];
        [self DeleteUserFollowWithFollowID:list.UserId];
    }
}
...
#pragma mark 用户取消关注
-(void)DeleteUserFollowWithFollowID:(NSString *)followID{
