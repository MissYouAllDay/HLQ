//
//  YPSupplierInfoAddYouhuiController.m
//  HunQingYH
//
...
#import "YPSupplierInfoAddYouhuiController.h"

@interface YPSupplierInfoAddYouhuiController ()<UITextViewDelegate>
...
@implementation YPSupplierInfoAddYouhuiController{
    UIView *_navView;
...
- (void)setupNav{
    ...
    [doneBtn setTitleColor:RGB(250, 80, 120) forState:UIControlStateNormal];
...
- (void)doneBtnClick{
    ...
    if (self.noteTextView.text.length > self.limitCount) {
        ...
        if (self.recordID.length > 0) {
            //修改
            [self UpdateFacilitatorBasicPreferences];
        }else{
            //添加
            [self CreateFacilitatorBasicPreferences];
        }
    }
}

#pragma mark - 网络请求
#pragma mark 新增服务商基础优惠
- (void)CreateFacilitatorBasicPreferences{
    
    [EasyShowLodingView showLoding];
    
    NSString *url = @"/api/HQOAApi/CreateFacilitatorBasicPreferences";
    
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    
    params[@"FacilitatorId"] = FacilitatorId_New;
    params[@"Discount"] = _noteTextView.text;
    ...
    [[NetworkTool shareManager] requestWithUrlStr:url withParams:params Success:^(NSDictionary *object) {
        ...
        if ([[[object valueForKey:@"Message"] valueForKey:@"Code"] integerValue] == 200) {
            ...
            [EasyShowTextView showSuccessText:@"添加优惠成功!" inView:self.view];
...
#pragma mark 修改服务商基础优惠
- (void)UpdateFacilitatorBasicPreferences{
    
    [EasyShowLodingView showLoding];
    
    NSString *url = @"/api/HQOAApi/UpdateFacilitatorBasicPreferences";
