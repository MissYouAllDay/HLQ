...
-(void)reloadGoodsInfo
{
    for (GoodsTypeModel *model in _dataSource) {
        if (model.selectIndex<0) {
            goodsInfo.promatLabel.text =[NSString stringWithFormat:@"请选择\"商品%@\"后，再添加商品数量",model.typeName];
            break;
        }
    }
    ...
    for (SizeAttributeModel *model in _model.sizeAttribute) {
        if ([model.value isEqualToString:str]) {
            sizeModel = model;
            if ([countView.countTextField.text intValue]>[sizeModel.Number intValue]) {
                countView.countTextField.text = [NSString stringWithFormat:@"%d",[sizeModel.Number intValue]];
            }else if ([countView.countTextField.text intValue]<[sizeModel.Number intValue])
            {
...
-(void)sure
{
    for (GoodsTypeModel *model in _dataSource) {
        if (model.selectIndex<0) {
            [JXUIKit showWithString:[NSString stringWithFormat:@"请选择\"商品%@\"后，再添加商品数量",model.typeName]];
            return;
        }
    }
    ...
    //判断库存
    if ([sizeModel.Number intValue]>0) {
        if (self.selectSize) {
...
-(void)add
{
    ...
    if (sizeModel) {
        if (count <[sizeModel.Number intValue]) {
            countView.countTextField.text = [NSString stringWithFormat:@"%d",count+1];
        }else{
            [EasyShowTextView showText:@":-( 库存不足了"];
        }
    }else
    {
...
-(void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    if (sizeModel) {
        if (count >[sizeModel.Number intValue]) {
            ...
            countView.countTextField.text = sizeModel.Number;
        }
    }else
    {
        if (count > [_model.totalStock intValue]) {
