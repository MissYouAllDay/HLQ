...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        //关闭按钮
        _closeButton = [JXUIKit buttonWithBackgroundColor:[UIColor whiteColor] imageForNormal:@"shopclose" imageForSelete:@"shopclose"];
...
//        _goodsPriceLabel.sd_layout.leftSpaceToView(_goodsImage, kSize(10)).rightSpaceToView(_closeButton, kSize(10)).heightIs(kSize(20)).topSpaceToView(_goodsTitleLabel, 0);
         _goodsPriceLabel.sd_layout.leftSpaceToView(_goodsImage, kSize(10)).rightSpaceToView(_closeButton, kSize(10)).heightIs(kSize(20)).topEqualToView(_goodsImage);
        //库存
...
{
    _model = model;
    [_goodsImage sd_setImageWithURL:[NSURL URLWithString:model.imageId]];
...
-(void)resetData:(SizeAttributeModel *)sizeModel
{
    ...
    if (sizeModel.imageId.length>0) {
        [_goodsImage sd_setImageWithURL:[NSURL URLWithString:sizeModel.imageId]];
    }else
        [_goodsImage sd_setImageWithURL:[NSURL URLWithString:_model.imageId]];
    
    _goodsCountLabel.text = [NSString stringWithFormat:@"库存：%@",sizeModel.Number];
