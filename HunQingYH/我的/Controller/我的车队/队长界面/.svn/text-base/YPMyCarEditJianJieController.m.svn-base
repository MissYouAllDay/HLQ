//
//  YPMyCarEditJianJieController.m
//  HunQingYH
//
//  Created by YanpengLee on 2017/9/13.
//  Copyright © 2017年 YanpengLee. All rights reserved.
//

#import "YPMyCarEditJianJieController.h"
...
@interface YPMyCarEditJianJieController ()<UITableViewDelegate,UITableViewDataSource,UIActionSheetDelegate,ZLPhotoPickerViewControllerDelegate,ZLPhotoPickerBrowserViewControllerDelegate,UITextViewDelegate,CJAreaPickerDelegate>
...
@implementation YPMyCarEditJianJieController{
    UIView *_navView;
    CGFloat _cellHeight;
    FMDatabase *dataBase;
//    MBProgressHUD *hud;
...
-(void)uploadIconImgRequest{
    ...
    [BANetManager ba_uploadImageWithEntity:imageEntity successBlock:^(id response) {
        NSLog(@"修改车队 返回：====%@",response);
        
        [self UpUserSupplierInfoWithIconID:[response objectForKey:@"Inform"]];
...
#pragma mark 供应商修改自己详细信息
- (void)UpUserSupplierInfoWithIconID:(NSString *)iconID{
    
    [EasyShowLodingView showLoding];
    
    NSString *url = @"/api/HQOAApi/UpUserSupplierInfo";
    ...
    params[@"OwnedCompany"] = @"";
    params[@"Adress"] = @"";
    params[@"IsSearch"] = @"-1";//0不可以、1可以、-1不修改
    params[@"Headportrait"] = iconID;//为空不修改
    
    params[@"TrueName"] = self.titleTF.text;
