    ...
    if (indexPath.row == 0) {
        ...
        if (indexPath.row == 1) {
            ...
            if ([self.isAddHotel integerValue] == 1) {//未预定
                ...
                cell.titleLabel.text = @"邀请人 (非必填)";
                cell.inputTF.placeholder = @"输入邀请人手机号";
                cell.inputTF.enabled = YES;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 3) {
        //身份
        UIActionSheet *sheet = [[UIActionSheet alloc]initWithTitle:@"请选择身份" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"新郎",@"新娘", nil];
...
- (void)upBtnClick{
    ...
    if (self.nameStr.length == 0) {
        ...
        [EasyShowTextView showText:@"请选择身份"];
    }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (actionSheet.tag == 1003) {
        ...
        if (buttonIndex == 0) {
            NSLog(@"新郎");
            //        self.shenfenTF.text = @"新郎";
            self.shenfenStr = @"新郎";
            
        }else if (buttonIndex == 1){
            NSLog(@"新娘");
            //        self.shenfenTF.text = @"新娘";
            self.shenfenStr = @"新娘";
        }
...
- (void)AddNewInformation{
    ...
    params[@"ObjectTypes"] = @"0";//0提交自己(我要结婚), 1提交朋友(朋友结婚)
    
    if (self.inviterPhone.length == 0) {
        params[@"SubmittingPhone"] = @"";//提交者手机号 3-5 改为邀请人手机号
    }else{
        params[@"SubmittingPhone"] = self.inviterPhone;//提交者手机号 3-5 改为邀请人手机号
    }
    ...
    if ([self.shenfenStr isEqualToString:@"新郎"]) {
        params[@"GroomName"] = self.nameStr;
        params[@"GroomPhone"] = self.phoneStr;
        params[@"BrideName"] = @"";
        params[@"BridePhone"] = @"";
    }else if ([self.shenfenStr isEqualToString:@"新娘"]){
        params[@"GroomName"] = @"";
        params[@"GroomPhone"] = @"";
