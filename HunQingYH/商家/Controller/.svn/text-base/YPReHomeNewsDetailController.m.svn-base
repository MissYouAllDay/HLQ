//
//  YPReHomeNewsDetailController.m
//  HunQingYH
//
...
@interface YPReHomeNewsDetailController ()<UITableViewDelegate,UITableViewDataSource>

@property (nonatomic, strong) UITableView *tableView;

@end

@implementation YPReHomeNewsDetailController{
    UIView *_navView;
    UIButton *_backBtn;
    UILabel *_titleLabel;
}
...
//        make.left.mas_equalTo(_backBtn.mas_right).offset(5);
//        make.right.mas_equalTo(_navView.mas_right).offset(-5);
    }];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    NSArray *arr = [self.articleModel.Imgs componentsSeparatedByString:@","];
    if (self.articleModel.Imgs.length > 0) {
        return 2 + arr.count;
    }else{
        return 2;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        if (self.articleModel.Content.length > 0) {
            cell.descLabel.text = self.articleModel.Content;
        }else{
...
//    }else if (indexPath.row==2){
        ...
        return [self getHeighWithTitle: self.articleModel.Content font:kFont(17) width:ScreenWidth]+50;
    }else{
        NSArray *arr = [self.articleModel.Imgs componentsSeparatedByString:@","];
        NSString *str = arr[indexPath.row - 2];
        /**
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (offsetY > NAVBAR_COLORCHANGE_POINT)
    {
        ...
        _titleLabel.width =ScreenWidth-80;
