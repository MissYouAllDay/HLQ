...
- (void)LQPhotoPicker_initPickerView{
    ...
    //添加图片提示
    addImgStrLabel = [[UILabel alloc]initWithFrame:CGRectMake(120, 50, 70, 20)];
...
{
    return CGSizeMake(([UIScreen mainScreen].bounds.size.width-NAVIGATION_BAR_HEIGHT) /3 ,([UIScreen mainScreen].bounds.size.width-NAVIGATION_BAR_HEIGHT) /3);
}
...
- (void)changeCollectionViewHeight{
    ...
    if (_collectionFrameY) {
        _pickerCollectionView.frame = CGRectMake(0, _collectionFrameY, [UIScreen mainScreen].bounds.size.width, (((float)[UIScreen mainScreen].bounds.size.width-NAVIGATION_BAR_HEIGHT) /3.0 +20.0)* ((int)(_LQPhotoPicker_selectedAssetArray.count)/3 +1)+20.0);
    }
    else{
        _pickerCollectionView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, (((float)[UIScreen mainScreen].bounds.size.width-NAVIGATION_BAR_HEIGHT) /3.0 +20.0)* ((int)(_LQPhotoPicker_selectedAssetArray.count)/3 +1)+20.0);
    }
...
- (void)LQPhotoPicker_updatePickerViewFrameY:(CGFloat)Y{
    ...
    _collectionFrameY = Y;
    _pickerCollectionView.frame = CGRectMake(0, Y, [UIScreen mainScreen].bounds.size.width, (((float)[UIScreen mainScreen].bounds.size.width-NAVIGATION_BAR_HEIGHT) /3.0 +20.0)* ((int)(_LQPhotoPicker_selectedAssetArray.count)/3 +1)+20.0);
}
