...
- (void)setupOtherViews {
    ...
    UILabel *lab1 = [self.itemsArrayM lastObject];
    CGFloat scrollSizeWidht = self.configration.itemLeftAndRightMargin + CGRectGetMaxX(lab1.frame);
    if (scrollSizeWidht < self.scrollView.yn_width) {//不超出宽度
        ...
        if (self.configration.aligmentModeCenter && left >= 0) {
            [self.itemsArrayM enumerateObjectsUsingBlock:^(UILabel  * label, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                UILabel *lab1 = [self.itemsArrayM lastObject];
            self.scrollView.contentSize = CGSizeMake(left + CGRectGetMaxX(lab1.frame), self.scrollView.yn_height);
            ...
            if (!self.configration.scrollMenu) {
                [self.itemsArrayM enumerateObjectsUsingBlock:^(UILabel  * label, NSUInteger idx, BOOL * _Nonnull stop) {
                    ...
                    UILabel *lab1 = [self.itemsArrayM lastObject];
                self.scrollView.contentSize = CGSizeMake(CGRectGetMaxX(lab1.frame), self.scrollView.yn_height);
