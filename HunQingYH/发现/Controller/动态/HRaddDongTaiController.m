...
@interface HRaddDongTaiController ()<HXPhotoViewDelegate>
{
    UIView *navView;
    //备注文本View高度
...
@implementation HRaddDongTaiController
#pragma mark - 隐藏导航条
...
- (void)initViews{
    ...
    //scrollView
    self.scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, NAVIGATION_BAR_HEIGHT+1, ScreenWidth, ScreenHeight-NAVIGATION_BAR_HEIGHT-1-50)];
    ...
    [fabuBtn setBackgroundColor:MainColor];
    [fabuBtn setTitle:@"发布" forState:UIControlStateNormal];
    [fabuBtn setImage:[UIImage imageNamed:@"fabujian"] forState:UIControlStateNormal];
    ...
    // button标题的偏移量
    fabuBtn.titleEdgeInsets = UIEdgeInsetsMake(0, -fabuBtn.imageView.bounds.size.width+2, 0, fabuBtn.imageView.bounds.size.width);
    // button图片的偏移量
    fabuBtn.imageEdgeInsets = UIEdgeInsetsMake(0, fabuBtn.titleLabel.bounds.size.width, 0, -fabuBtn.titleLabel.bounds.size.width);
    ...
    // 照片或视频选择器
    self.PhotoOrVideoView = [[HXPhotoView alloc] initWithFrame:CGRectMake(kPhotoViewMargin, CGRectGetMaxY(_noteTextView.frame)+15, self.view.frame.size.width - kPhotoViewMargin * 2, 0) WithManager:self.PhotoOrVideoManager];
...
- (void)updateViewsFrame{
    ...
    //公告编辑框
    _noteTextView.frame = CGRectMake(0, 0, ScreenWidth , noteTextHeight);

  
    
    //photoPicker
    self.PhotoOrVideoView.frame =CGRectMake(kPhotoViewMargin, CGRectGetMaxY(self.noteTextView.frame)+10, self.view.frame.size.width - kPhotoViewMargin * 2, 0);
    
    allViewHeight =_noteTextView.frame.size.height+self.PhotoOrVideoView.frame.size.height+300;
...
//              UIImage *image = [UIImage imageWithData:[NSData dataWithContentsOfURL:photoModel.fileURL]];
...
//        for (HXPhotoModel *photoModel in self.selectImageArray) {
//
//            NSLog(@"自己家多久%lf",photoModel.imageSize.height);
//            if (photoModel.type == HXPhotoModelMediaTypeCameraPhoto){
//                [self.upXCArray addObject:photoModel.previewPhoto];
//            }
//            photoMode
...
-(void)fabuBtnClick{
   // NSLog(@"发布");
    ...
    if (self.upXCArray.count ==0) {
        [EasyShowTextView showText:@"添加几张照片再发布吧"];
    }else{
